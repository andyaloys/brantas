...
public static class RegencyCatalog
{
    private static readonly Dictionary<string, string[]> ProvinceRegencies = new(StringComparer.OrdinalIgnoreCase)
    {
        ["Aceh"] =
        [
    ...
    public static string GetRegencyName(string provinceName, int index)
    {
        if (ProvinceRegencies.TryGetValue(provinceName, out var regencies) && regencies.Length > 0)
        {
            return regencies[index % regencies.Length];
        }
        return $"{provinceName} {(index + 1):00}";
    }

    public static string ResolveSyntheticName(string name)
    {
        if (string.IsNullOrWhiteSpace(name)) return name;

        // Cek pola "{Provinsi} {NN}" (misal "Papua Pegunungan 04", "Banten 02")
        var parts = name.Trim().Split(' ');
        if (parts.Length >= 2 && int.TryParse(parts[^1], out var number))
        {
            var provinceName = string.Join(" ", parts[..^1]);
            return GetRegencyName(provinceName, number - 1);
        }
